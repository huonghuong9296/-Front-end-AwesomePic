$(document).ready(function(){	
	$("#product-tags").attr("class", "active");
	...
	var dataTable = $('#table-admin').DataTable({
		...
		language: {
			search: "_INPUT_",
			searchPlaceholder: "Product Tags"
		},
		responsive: true,
		"pageLength": 10,
		"order":[],
		"ajax":{
			url:"../controller/product_tag_ctrl.php",
			type:"POST",
	...
	$("#addModal").on('submit','#addForm', function(event){
		...
		if (($('#tag_id').val()).trim() == "") {
			$('#tag_idErr').text("* Tag ID is required.");
			errClient = true;
		}
		else if (!validateID($('#tag_id').val())) {
			$('#tag_idErr').text("* Invalid Tag ID format!");
			errClient = true;
		}
		...
		if (errClient == false) {
			...
			$.ajax({
				...
				success:function(result){			
					...
					if(result.tag_idErr != ""){
						$('#tag_idErr').text("*"+result.tag_idErr);
					}
					
					if(result.error == "error"){
						alert("Add new product tag failed!");
					}
					else if(result.error == "success"){
						$('#addForm')[0].reset();
						$('#addModal').modal('hide');
						dataTable.ajax.reload();
						alert("Add new product tag success!");
					}
					else {
						alert("Error handling service!");
					}
	...
	$("#table-admin").on('click', '.edt-btn-admin', function(){
		...
		$('#tag_id_old').val(data[3]);
		$('#tag_old').val(data[3]+". "+data[4]);
	...
	$("#edtModal").on('submit','#edtForm', function(event){
		...
		if (($('#tag_id_old').val()).trim() == "") {
			$('#tag_id_oldErr').text("* Old Tag ID is required.");
			errClient = true;
		}
		else if (!validateID($('#tag_id_old').val())) {
			$('#tag_id_oldErr').text("* Invalid Old Tag ID format!");
			errClient = true;
		}
		...
		if (($('#tag_id_new').val()).trim() == "") {
			$('#tag_id_newErr').text("* New Tag ID is required.");
			errClient = true;
		}
		else if (!validateID($('#tag_id_new').val())) {
			$('#tag_id_newErr').text("* Invalid New Tag ID format!");
			errClient = true;
		}
		// end Check client input
		...
		if (errClient == false) {
			...
			$.ajax({
				...
				success:function(result){			
					...
					if(result.tag_id_oldErr != ""){
						$('#tag_id_oldErr').text("*"+result.tag_id_oldErr);
					}
					if(result.product_id_newErr != ""){
						$('#product_id_newErr').text("*"+result.product_id_newErr);
					}
					if(result.tag_id_newErr != ""){
						$('#tag_id_newErr').text("*"+result.tag_id_newErr);
					}

					if(result.error == "error"){
						alert("Edit product tag failed!");
					}
					else if(result.error == "success"){
						$('#edtForm')[0].reset();
						$('#edtModal').modal('hide');
						dataTable.ajax.reload();
						alert("Edit product tag success!");
					}
					else {
						alert("Error handling service!");
					}
	...
	$("#table-admin").on('click', '.del-btn-admin', function(){
		...
		$('#tag_id_del').val(data[3]);
		$('#tag_del').val(data[3]+". "+data[4]);
	...
	$("#delModal").on('submit','#delForm', function(event){
		...
		if (($('#tag_id_del').val()).trim() == "") {
			$('#tag_id_delErr').text("* Tag ID is required.");
			errClient = true;
		}
		else if (!validateID($('#tag_id_del').val())) {
			$('#tag_id_delErr').text("* Invalid Tag ID format!");
			errClient = true;
		}
		...
		if (errClient == false) {
			...
			$.ajax({
				...
				success:function(result){			
					...
					if(result.tag_idErr != ""){
						$('#tag_id_delErr').text("*"+result.tag_idErr);
					}

					if(result.error == "error"){
						alert("Delete product tag failed!");
					}
					else if(result.error == "success"){
						$('#delForm')[0].reset();
						$('#delModal').modal('hide');
						dataTable.ajax.reload();
						alert("Delete product tag success!");
					}
					else {
						alert("Error handling service!");
					}
