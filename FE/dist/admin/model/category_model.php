...
class Data extends Dbconfig {	
	...
	public function dataList(){		
		...
		$colArr = ['id', 'id', 'NAME', 'description', 'created_date', 'updated_date'];
		...
		$result = mysqli_query($this->dbConnect, $query);
		if (!$result) {
			$error = " Error select category search: ".$this->dbConnect->error;
		...
		if (!$result1) {
			$error = " Error select category numRows: ".$this->dbConnect->error;
			echo json_encode($error);
		}
	...
	public function addData(){
		...
		else if (!preg_match("/^[a-zA-Z0-9\s]{1,32}$/",$name)) {
			$error['nameErr'] = " Only letters, numbers and white space allowed. Limited to 32 characters.";
			$error['error'] = "error";
		}
		...
		if ($error['error'] == "success") {
			...
			if (!$query_run) {
				$error['error'] = "error";
				$error['queryErr'] = " Error query insert category: ".$this->dbConnect->error;
			}
		}
	...
	public function updateData(){
		...
		if ($error['error'] == "success") {
			...
			if (!$query_run) {
				$error['error'] = "error";
				$error['queryErr'] = " Error query update category: ".$this->dbConnect->error;
			}
		}
	...
	public function deleteData(){
		...
		if ($error['error'] == "success") {
			...
			if (!$query_run) {
				$error['error'] = "error";
				$error['queryErr'] = " Error query update to deleto category: ".$this->dbConnect->error;
			}
		}
